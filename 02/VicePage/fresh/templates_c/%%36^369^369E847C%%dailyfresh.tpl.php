<?php /* Smarty version 2.6.30, created on 2018-04-25 08:40:08
         compiled from dailyfresh.tpl */ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>华北站每日鲜|华北站每日鲜【品牌 价格 产地 网购】_每日鲜-中粮我买网</title>
    <link rel="stylesheet" href="font-awesome-4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="css/base.css">
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/dailfresh.css">
</head>
<body>
    <!-- 头部用户操作 -->
    <div class="header">
        <div class="topnav">
            <ul class="location-City">
                <li class="icon-House"></li>
                <li class="now-City">当前城市:</li>
                <li class="City">巴丹吉林</li>
                <li class="icon-down"></li>
            </ul>
            <ul class="loginbar clearfix">
                <li class="msg">hi,欢迎来到我买网！</li>
                <li><a href="javascript:;">登陆</a></li>
                <li><a href="javascript:;">注册</a></li>
            </ul>
            <ul class="nav-service">
                <li class="my-Account">我的账户<span></span></li>
                <li class="Shopping"><span class="Shopping-Cart"></span>购物车 <e style="color: #8ab825">0</e>件</li>
                <li class="lineY">|</li>
                <li class="people-service">客户服务<span></span></li>
                <li class="lineY">|</li>
                <li>大客户</li>
                <li class="lineY">|</li>
                <li class="icbc"><span></span></li>
                <li class="icbc2"></li>
                <li class="cmcc"><span></span></li>
            </ul>
        </div>
    </div>
    <!-- 右边侧栏 -->    
    <div class="right-sidebar">
            <div>
                <div class="Shopping-Cart-Right">
                    <span class="Shopping-Cart-icon"></span>
                    购物车
                    <span class="Shopping-Cart-num">0</span>
                </div>
            </div>
            <ul>
                <li>
                    <div>我的收藏</div>
                </li>
                <li>
                    <div>账户中心</div>
                </li>
                <li>
                    <div>
                        在线充值
                    </div>
                </li>
                <li>
                    <div style="width: 154px;height: 272px;margin-left: -80px;margin-top: -217px;background: url(./images/img/right-sidebar-code-bg.png);">
                       <img src="./images/img/right-sidebar-code.jpg" alt="" style="margin-left: -2px;margin-top: 70px;">
                    </div>
                </li>
                <li>
                    <div>
                        顶部&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    </div>
                </li>
            </ul>
        </div>
    <!-- LOGO层 -->
    <div class="search-content">
        <div>
            <img src="./images/img/logo.png" alt="">
            <img src="./images/img/Yogurt.jpg" alt="">
            <div class="search-Div">
                <input type="text" class="searchi-Ipt" placeholder="银联扫码支付 满100立减20">
                <button class="search-btn">搜索</button>
                <span class="search-two"><a>高级搜索</a></span>
                <ul>
                    <li><a href='javascript:;'>牛奶</a></li>
                    <li><a href='javascript:;'>每日坚果</a></li>
                    <li><a href='javascript:;'>火龙果</a></li>
                    <li><a href="javascript:;">红颜草莓</li>
                    <li><a href="javascript:;">冰淇淋</a></li>
                    <li><a href="javascript:;">实物我买卡</a></li>
                </ul>
                <div class="icon-search">
                </div>
            </div>
            <img src="./images/img/phone-code.jpg" alt="" class="phone-code">
        </div>
    </div>
    <!-- 网站导航div -->
    <div class="web-nav">
        <div>
            <span class="pro-Type">
                全部商品分类
            </span>
            <ul>
                <li><a href="javascript:;">团购</a></li>
                <li><a href="javascript:;">每日鲜</a></li>
                <li><a href="javascript:;">我买酒</a></li>
                <li><a href="javascript:;">我买严选</a></li>
                <li><a href="javascript:;">包邮直送</a></li>
                <li><a href="javascript:;">全球购</a></li>
            </ul>
            <span class="mycart">
                <span class="mycart-cart-icon"></span>
                <a href="javascript:;">购物车</a>
                <span class="mycart-cart-top"></span>
                <div class="mycart-div">
                    <img src="images/img/cart-no.jpg" alt="">
                    <p>购物车中还没有商品,赶紧选购吧！</p>
                    <a href="javascript:;" id="xxxxx-link">去首页看看</a>
                </div>
            </span>
        </div>
    </div>
    <div class="main">
        <div class="img">
            <img src="./images/img/img.jpg" alt="">
        </div>
        <div class="top-nav">
            <div class="search">
                <ul class="search-list clearfix">
                    <li class="item item1">
                        <a href="javascript:;">食品</a>
                    </li>
                    <li>
                        <i class="fa fa-angle-right"></i>
                    </li>
                    <li class="item item2"> 
                        <a href="javascript:;">每日鲜</a>
                        <i class="fa fa-angle-down"></i>
                        <i class="fa fa-angle-up"></i>
                        <div class="detail detail1">
                            <a href="javascript:;">华东站每日鲜</a>
                            <a href="javascript:;">华南站每日鲜</a>
                            <a href="javascript:;">华西站每日鲜</a>
                            <a href="javascript:;">华北站每日鲜</a>
                        </div>
                    </li>
                    <li>
                        <i class="fa fa-angle-right"></i>
                    </li>
                    <li class="item item3">
                        <a href="javascript:;">华北站每日鲜</a>
                        <i class="fa fa-angle-down"></i>
                        <i class="fa fa-angle-up"></i>
                        <div class="detail detail2">
                            <a href="javascript:;">主食</a>
                        </div>
                    </li>
                    <li>
                        <i class="fa fa-angle-right"></i>
                    </li>
                    <li class="item4 item">
                        <input type="text" value="在当前条件下搜索" class="searched">
                        <i class="fa fa-search"></i>
                    </li>
                    
                </ul>
            </div>
            <div class="menu">
                <div class="list list1">
                    <div class="list-l">单件重量</div>
                    <div class="list-r">
                        <a href="javascript:;">250g及以下</a>
                        <a href="javascript:;">500g及以上</a>
                        <a href="javascript:;">251g-499g</a>
                        <span class="select">+多选</span>
                    </div>
                </div>
                <div class="list list2">
                    <div class="list-l">类别</div>
                    <div class="list-r">
                        <a href="javascript:;">西式</a>
                        <a href="javascript:;">中式</a>
                        <span class="select">+多选</span>
                    </div>
                </div>
                <div class="list list3">
                    <div class="list-l">包装</div>
                    <div class="list-r">
                        <a href="javascript:;">简装</a>
                    </div>
                </div>
                
            </div>
        </div>
        <div class="content clearfix">
            <div class="slidebar clearfix">
                <a href="javascript:;" class="slide">
                    展开侧栏
                    <i class="fa fa-angle-right"></i>
                </a>    
                <div class="hiden">
                    <div class="close-slide">
                        <a href="javascript:;" class="closed">
                            <i class="fa fa-angle-right"></i>
                            收起侧栏
                        </a>
                    </div>
                <div class="msg">
                    搜索了最终要购买的
                </div>
            </div>
            </div>
            <div class="select clearfix">
                <div class="select-top clearfix">
                    <div class="top-cont">
                        <div class="link">
                            <a href="javascript:;" class="active">综合</a>
                            <a href="javascript:;">销量</a>
                            <a href="javascript:;">价格</a>
                            <a href="javascript:;">评论数</a>
                            <a href="javascript:;">时间</a>
                        </div>
                        <div class="search">
                            <a href="javascript:;">最低价</a>
                            <span>-</span>
                            <a href="javascript:;">最高价</a>
                            <button>确定</button>
                        </div>
                        <div class="total">总共36件相关商品1/1</div>
                    </div>
                </div>
                <div class="select-bottom">
                    <ul class="list">
                        <li><a href="javascript:;"><i class="active"></i>有货</a></li>
                        <li><a href="javascript:;"><i></i>自营</a></li>
                        <li><a href="javascript:;"><i></i>产地直送</a></li>
                        <li><a href="javascript:;"><i></i>跨境购</a></li>
                        <li><a href="javascript:;"><i></i>促销</a></li>
                    </ul>
                </div>
            </div>
            <div class="product-list">
                <ul class="list clearfix">
                    <?php $_from = $this->_tpl_vars['product']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }$this->_foreach['name'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['name']['total'] > 0):
    foreach ($_from as $this->_tpl_vars['key'] => $this->_tpl_vars['value']):
        $this->_foreach['name']['iteration']++;
?>
                        <li class="item">
                            <a href="javascript:;"><img src="./images/<?php echo $this->_tpl_vars['value']['img']; ?>
"></a>
                            <span class="price"><?php echo $this->_tpl_vars['value']['price']; ?>
</span>
                            <a href="javascript:;" class="info"><?php echo $this->_tpl_vars['value']['msg']; ?>
</a>
                            <p class="buy">
                                <a href="javascript:;" class="buy1">已评价<span><?php echo $this->_tpl_vars['value']['charge']; ?>
次</span></a>
                                <a href="javascript:;" class="buy2"><i></i> 收藏</a>
                                <a href="javascript:;" class="buy3"><i></i> 加入购物车</a>
                            </p>
                            <span class="mark mrx">每日鲜</span>
                            <span class="mark qg">抢购</span>
                        </li>
                    <?php endforeach; endif; unset($_from); ?>
                </ul>
            </div> 
            <div class="page clearfix">
                <a href="javascript:;" class="public">
                    <i class="fa fa-angle-left"></i> 上一页
                </a>
                <div class="page-num public">1</div>
                <a href="javascript:;" class="public">
                    下一页 <i class="fa fa-angle-right"></i>
                </a>
                <div class="page-total">共1页</div>
                <div>到第</div>
                <input type="text" value="1" class="public">
                <div>页</div> 
                <button class="public btn">确定</button>
            </div>
        </div>
    </div>
    <div class="footer">
            <div class="footer-top">
                <div class="width clearfix">
                <dl class="first">
                    <dt><a href="#"><h3>新手上路</h3></a></dt>
                    <dd><a href="#">新用户演示</a></dd>
                    <dd><a href="#">会员积分计划</a></dd>
                    <dd><a href="#">会员等级规则</a></dd>
                    <dd><a href="#">售后服务政策</a></dd>
                    <dd><a href="#">我买网发票制度</a></dd>
                </dl>
                <dl>
                    <dt><a href="#"><h3>付款方式</h3></a></dt>
                    <dd><a href="#">支付方式</a></dd>
                    <dd><a href="#">支付常见问题</a></dd>
                    <dd><a href="#">激活礼品卡</a></dd>
                    <dd><a href="#">我买卡</a></dd>
                    <dd><a href="#">电子礼品卡说明及使用</a></dd>
                </dl>
                <dl>
                    <dt><a href="#"><h3>配送信息</h3></a></dt>
                    <dd><a href="#">配送运费信息</a></dd>
                    <dd><a href="#">生鲜速配运费标准</a></dd>
                    <dd><a href="#">订单处理时间</a></dd>
                    <dd><a href="#">验收与拒绝政策</a></dd>
                    <dd><a href="#">用卷订单结算说明</a></dd>
                </dl>
                <dl>
                    <dt><a href="#"><h3>我买提示</h3></a></dt>
                    <dd><a href="#">找回密码</a></dd>
                    <dd><a href="#">联系我们</a></dd>
                    <dd><a href="#">常见热点问题</a></dd>
                    <dd><a href="#">关于跨境购</a></dd>
                </dl>
        
                <div class="ervma">
                    <dl class="fl">
                    <dt>我买网APP</dt>
                    <dd><img src="./images/img/App-erweima.jpg" alt=""></dd>
                    </dl>
                    <dl class="fl">
                    <dt>我买网微信</dt>
                    <dd><img src="./images/img/weixin-erweima.jpg" alt=""></dd>
                    </dl>
                </div>
                </div> 
            </div>
            <div class="footer-bottom">
                <div class="footer-link">
                    <a href="#">关于我们</a><span>|</span>
                    <a href="#">免责条款</a><span>|</span>
                    <a href="#">中粮集团</a><span>|</span>
                    <a href="#">友情链接</a><span>|</span>
                    <a href="#">搜索热词</a><span>|</span>
                    <a href="#">客服电话：400-005-5678</a>
                </div>
                <div class="illustration clearfix">
                    <div class="pic fl">
                        <a href="#"><img src="./images/img/footer-logo.jpg"></a>
                    </div>
                    <div class="illus-detail fl">
                        <p>Copyright©2009-2018 womai.com.hk All Rights Reserved 中粮海优（北京）有限公司版权所有</p>
                        <p>中粮海优（北京）有限公司 北京市朝阳区朝阳门南大街8号18F-09室</p>
                        <p><a href="#"><i class="anbei"></i><span> 京公网安备 11010102000458号 </span></a><span> 京ICP备 13003995号</span></p>
                        <p>营业执照（统一社会信用代码）91110000055590239L 食品经营许可证：JY11105020028185</p>
                    </div>
                    <div class="bottom-icon"><a href="#"><i class="xin"></i></a></div>
                    </div>
                </div>  
            </div>
    </div>
    <script src="./js/jquery-1.12.4.min.js"></script>
    <script src="./js/jquery.lazyload.js"></script>
    <script src="./js/dailyfresh.js"></script>
</body>
</html>